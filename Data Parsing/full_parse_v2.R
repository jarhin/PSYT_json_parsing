...
#table for cohort analysis
cohort <- practice_performances_engagement %>%
  group_by(UUID) %>% 
  summarise(
    first_practice = min(first_createdAt),
    last_practice = max(first_createdAt)
    )
...
output_dir = "~/PSYT/Mindlab Report/Data Parsing/Parsed CSV v2/"
...
for(i in ls())
{
  write.table(
    get(i), 
    file = paste0(output_dir, i, "_v2.csv"), 
    quote = TRUE, 
    dec = ".", 
    sep =",", 
    na = "", 
...
cohort_analysis_practice <- practice_performances_engagement %>% 
  left_join(cohort) %>%
  mutate(
    start_cohort = format(first_practice, format = "%Y-%m"),
    actualDay_week = floor(actualDay/7) + 1 
    ) %>% 
  left_join(practice) %>% 
  group_by(start_cohort, actualDay_week) %>% 
  filter(actualDay_week >= 1) %>%
  summarise(
    count_practices = n(),
    count_meditations = sum(is_Meditation),
    count_distinct_users = n_distinct(UUID)
    )

cohort_first_week_users <- cohort_analysis_practice[cohort_analysis_practice$actualDay_week == 1, c("start_cohort", "count_distinct_users")]
cohort_first_week_users[["initial_number_users"]] <- cohort_first_week_users[["count_distinct_users"]]
cohort_first_week_users[["count_distinct_users"]] <- NULL

cohort_analysis_practice_full <- cohort_analysis_practice %>% 
  left_join(cohort_first_week_users) %>%
  mutate(
    ratio_initial_users = count_distinct_users/initial_number_users,
    practices_per_user = count_practices/ count_distinct_users,
    meditations_per_user = count_meditations/ count_distinct_users
    )
